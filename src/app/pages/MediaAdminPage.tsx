...
export function MediaAdminPage({ onNavigate }: MediaAdminPageProps) {
  ...
  const addAssets = async (incoming: MediaAsset[]) => {
    if (incoming[0]?.id.startsWith("replace-")) {
      const draft = { ...incoming[0], id: incoming[0].id.slice("replace-".length) };
      if (isSupabaseConfigured && selected) {
        const uploaded = await uploadMediaFile(await dataUrlToFile(draft), { ...draft, placement: selected.placement }, selected.id);
    ...
    if (isSupabaseConfigured) {
      const uploaded = await Promise.all(incoming.map(async (asset) => uploadMediaFile(await dataUrlToFile(asset), asset)));
